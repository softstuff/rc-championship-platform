...
//        }
    ...
    List<DecoderMessage> processInputBuffer(ByteBuffer readBuffer, int bytes) {

        List<DecoderMessage> messages = new LinkedList<>();
        int from = 0;
        while (from < bytes) {
            ...
            int endOfRecord = findPositition(Const.EOR, startOfRecord, bytes, readBuffer) +1;
            if (endOfRecord <= 1) {
                String hexBinary = DatatypeConverter.printHexBinary(readBuffer.array());
    ...
    private void fireDataReadEvent(List<DecoderMessage> readMessages) {
        for (DecoderMessage message : readMessages) {
            LOG.log(Level.FINER, "recived a message {0} from {1}", new Object[]{message, this});
            listeners.forEach(listerner -> listerner.recived(message));
        }
    }
...
//                    String dstr = DatatypeConverter.printHexBinary(array);
                    if (bytes == 0) {
                        Thread.sleep(100);
                        continue;
                    }
                    ...
                    List<DecoderMessage> messages = processInputBuffer(data, bytes);
                    fireDataReadEvent(messages);
